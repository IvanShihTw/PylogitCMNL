...
def create_constraining_vars_matrix(long_form,
                                    constrained_vars_name):
    ##########
    # Actually create the design matrix
    ##########
    # Create a list of the columns of constraining variables
    constraining_vars = []
    # Create a list of variable names
    var_names = []

    # Create the columns of the design matrix based on the specification dict.
    for variable in constrained_vars_name:
        # specification = constrained_vars_name[variable]
        # for group in specification:

            # if isinstance(group, Number):
            #     # Create the variable column
            #     new_col_vals = (long_form[variable].values)
            #     independent_vars.append(new_col_vals)
            #     # Create the column name
            #     var_names.append("{}_{}".format(variable, group))

            # else:  
            # the group is a list, tuple, or rang
        # Create the variable column
        constraining_vars.append(long_form[variable].values)
        # Create the column name
        #var_names.append("{}_{}".format(variable, str(group)))

    # Create the constraining_var_matrix
    constraining_var_matrix = np.hstack(tuple(x[:, None] for x in constraining_vars))

    # # Use the list of names passed by the user, if the user passed such a list
    # if constrained_vars_name is not None:
    #     var_names = []
    #     for value in constrained_vars_name.values():
    #         if isinstance(value, str):
    #             var_names.append(value)
    #         else:
    #             for inner_name in value:
    ...
    return constraining_var_matrix, constrained_vars_name #var_names
...
def convert_wide_to_long_without_alt_vars(wide_data,
                                          ind_vars,
                                          availability_vars,
                                          obs_id_col,
                                          choice_col,
...
def convert_constraining_names_to_positions(constraining_names, ind_var_names):
    """
    Parameters
    ----------
    mixing_names : list.
    ...
    -------
     list.
         ...
         the elements in mixing name, in the `ind_var_names` list.
    """
    return [ind_var_names.index(name) for name in constraining_names]
