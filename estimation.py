...
class EstimationObj(object):
    ...
    def __init__(self,
        ...
        # Store CMNL pointers to needed objects#########################
        self.constraining_vars_vals = model_obj.constraining_vars_vals
        self.constraining_vars_names = model_obj.constraining_vars_names
        self.cutoff_tol = model_obj.cutoff_tol
        self.upper_limit_input = model_obj.upper_limit_input
        ...
        # Store reference to what 'zero vector' is for this model / dataset
        self.cmnl_ps_zero_vector = cmnl_ps_zero_vector

        # Specify num of index and constraining vars
        self.num_index = model_obj.num_index
        self.num_constrained = model_obj.num_constrained
    ...
    def cmnl_convenience_split_params(self, param1, param2, return_all_types=False):
        """
        Splits parameter vector into shape, intercept, and index parameters.
        ...
        tuple. Will have 4 or 3 elements based on `return_all_types`.
        """
        return self.split_params(param1, 
                                 param2,
                                 self.rows_to_alts,
                                 self.design,
                                 return_all_types=return_all_types)
    
    def cmnl_final_split(self, final_params):

        final_beta = final_params[ : self.num_index]
        final_phi_scale = final_params[-1 * self.num_constrained : ]

        return None, None, None, final_beta, final_phi_scale
    ...
    def cmnl_calc_neg_log_likelihood_and_neg_gradient(self, final_init_vals):
        """
        Calculates and returns the negative of the log-likelihood and the
        negative of the gradient. This function is used as the objective
        function in scipy.optimize.minimize.
        """
        beta_input = final_init_vals[0 : self.num_index]
        phi_scale_input = final_init_vals[ -1 * self.num_constrained : ]

        neg_log_likelihood = -1 * self.cmnl_convenience_calc_log_likelihood(betas=beta_input,
                                                                            phi_scale=phi_scale_input)
        neg_gradient = -1 * self.cmnl_convenience_calc_gradient(betas=beta_input,
                                                                phi_scale=phi_scale_input)
...
class LogitTypeEstimator(EstimationObj):
    ...
    def convenience_calc_probs(self, params):
        ...
        format probabilities for this model and dataset.
        """
        shapes, intercepts, betas, phi_scale = self.convenience_split_params(params)
    ...
    def cmnl_convenience_calc_probs(self, betas, phi_scale):
        ...
        format probabilities for this model and dataset.
        """
        shapes, intercepts, betas, phi_scale = self.cmnl_convenience_split_params(betas, phi_scale)
...
def calc_and_store_post_estimation_results(results_dict,
    ...
    results_dict["phi_scale"] = split_res[4]
...
def cmnl_calc_and_store_post_estimation_results(results_dict,
                                                estimator):
    """
    Calculates and stores post-estimation results that require the use of the
    systematic utility transformation functions or the various derivative
    ...
    # Add the estimated parameters to the results dictionary
    split_res = estimator.cmnl_final_split(final_params)
    ...
    # Get the probability of the chosen alternative and long_form probabilities
    chosen_probs, long_probs = estimator.cmnl_convenience_calc_probs(betas=split_res[3],
                                                                     phi_scale=split_res[4])
    ...
    #####
    results_dict["final_gradient"] =\
        estimator.cmnl_convenience_calc_gradient(betas=split_res[3],
                                                 phi_scale=split_res[4])
    results_dict["final_hessian"] =\
        estimator.cmnl_convenience_calc_hessian(betas=split_res[3],
                                                phi_scale=split_res[4])
    results_dict["fisher_info"] =\
        estimator.cmnl_convenience_calc_fisher_approx(betas=split_res[3],
                                                      phi_scale=split_res[4])
...
def cmnl_estimate(init_vals,
    ...
    if not just_point:
        # Perform preliminary calculations
        log_likelihood_at_zero =\
            cmnl_estimator.cmnl_convenience_calc_log_likelihood(betas = cmnl_estimator.zero_vector,
                                                                phi_scale = cmnl_estimator.cmnl_ps_zero_vector)

        initial_log_likelihood =\
            cmnl_estimator.cmnl_convenience_calc_log_likelihood(betas = init_vals,
                                                                phi_scale = phi_scale_init_vals)
    ...
    # Get the hessian fucntion for this estimation process
    hess_func = cmnl_estimator.calc_neg_hessian if use_hessian else None

    # Estimate the actual parameters of the model
    start_time = time.time()

    # connect two array of init_vals into 1 1D ndarray
    final_init_values = np.concatenate((init_vals, phi_scale_init_vals), axis=None)
    beta_bnds = [(None, None) for n in range(init_vals.size)]
    phi_scale_bnds = [(0,1) for n in range(phi_scale_init_vals.size)]
    bnds = beta_bnds + phi_scale_bnds

    results = minimize(cmnl_estimator.cmnl_calc_neg_log_likelihood_and_neg_gradient,
                       final_init_values,
                       method=method,
                       jac=True,
                       hess=hess_func,
...
# 
    if not just_point:
        ...
        # Calculate and store the post-estimation results
        results = cmnl_calc_and_store_post_estimation_results(results, cmnl_estimator)
