...
from .choice_tools import create_constraining_vars_matrix
...
def ensure_all_constraining_vars_are_in_the_name_dict(constraining_vars_names,
                                                      name_dict,
                                                      ind_var_names):                                                
    """
    Ensures that all of the variables listed in `constraining_vars` are present in
    `ind_var_names`. Raises a helpful ValueError if otherwise.

    Parameters
    ----------
    constraining_vars_names : list of strings, or None.
        Each string denotes a parameter is related to constrained behavior.
    name_dict : OrderedDict or None.
        Contains the specification relating column headers in one's data (i.e.
        the keys of the OrderedDict) to the index coefficients to be estimated
        based on this data (i.e. the values of each key).
    ...
    None.
    """
    if constraining_vars_names is None:
        return None

    # Determine the strings in constraining_vars that are missing from ind_var_names
    problem_names = [variable_name for variable_name in constraining_vars_names
                     if variable_name not in ind_var_names]
...
class MNDC_Model(object):
    ...
    mixing_vars : list, or None, optional.
        ...
        passed. Default == None.
    constraining_vars : list, or None, optional.
        All elements of the list should be strings. Each string should be
        present in the values of `names.values()` and they're associated
        variables should only be index variables (i.e. part of the design
        matrix). If `model_type == "CMNL"`, then `constraining_vars` must be
        passed. Default == None.
    ...
    def __init__(self,
        ...
        # Create the constraining matrix for this model
        ##########
        if cmnl_spec is not None:
            the_constraining_vars = create_constraining_vars_matrix(dataframe,
                                                                    cmnl_spec)               
        ...
        self.design = design_res[0]  #THE DESIGN MATRIX
        self.ind_var_names = design_res[1] #THE DESIGN NAME
        self.alt_id_col = alt_id_col
        self.obs_id_col = obs_id_col
        self.choice_col = choice_col
        self.specification = specification
        self.obs_IDs = dataframe[obs_id_col].values
        ...
        self.cmnl_spec = cmnl_spec
        if cmnl_spec is not None:
            constraining_vars_vals = the_constraining_vars[0] #CONSTRAINED VAR'S VALUE
            constraining_vars_names = the_constraining_vars[1] #CONSTRAINED VAR'S NAME
        else:
            constraining_vars_vals = None
            constraining_vars_names = None
        
        self.num_index = num_index
        self.num_constrained = num_constrained
        self.num_obs = len(set(self.obs_IDs.tolist()))

        #reshape constraining_val into shape(n*i*k)
        if cmnl_spec is not None:
            self.constraining_vars_vals = constraining_vars_vals.reshape(-1, num_constrained) 
            self.constraining_vars_names = constraining_vars_names
        else:
            self.constraining_vars_vals = None 
            self.constraining_vars_names = None            
        self.cutoff_tol = cutoff_tol 
        self.upper_limit_input = upper_limit_input

        final_phi_scale = []
        if cmnl_spec is not None:            
            for spec_index in np.arange(self.num_constrained):
                yy = 'PhiScale_{test_cmnl_spec}'.format(test_cmnl_spec=self.cmnl_spec[spec_index])
                final_phi_scale.append(yy)
        else:
            final_phi_scale = None
        self.phi_scale_names = final_phi_scale
    ...
    def store_fit_results(self, results_dict):
        ...
        if results_dict["phi_scale"] is not None:
            storage_args = [results_dict["phi_scale"],
                            "phi_scale_names",
                            "Phi_Scale_Param_{}",
                            all_names,
                            all_params,
                            "phi_scales",
                            "phi_scale_parameters"]
            storage_results = self._store_optional_parameters(*storage_args)
            all_names, all_params = storage_results
        else:
            self.phi_scales = None
    ...
    def predict(self,
        ...
        elif self.model_type == "CMNL":
            ##########
            # This condition accounts for the fact that Constrained Multinomial Logit models have
            # a different functional interface than the standard logit-type
            # models.
            ##########

            return cmcc.calc_probability(new_index_coefs,
                                         new_design,
                                         constraining_vars,
                                         upper_limit, 
                                         phi_scale, 
