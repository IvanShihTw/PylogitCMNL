...
# constraining_vars (n,i,k) shape(sum i over n,k)
# upper_limit (n,k)

def calc_nik_minus_nk(n_i_k,
                      n_k,
                      rows_to_obs):
    
    cmnl_r_to_o = rows_to_obs.toarray()
    split_ind_bande = []
    num_of_obs = cmnl_r_to_o.shape[1]
    for i in np.arange(num_of_obs):
        index = np.where(cmnl_r_to_o.T[i]==1)
        index_bae = [index[0]]
        split_ind_bande.append(index_bae)
    fnik=[]
    for i in np.arange(num_of_obs):
        b = np.asarray(split_ind_bande[i]).reshape(-1)
        nik = n_i_k[b[0]:b[-1]+1]
        fnik.append(nik)
    # calc n_i_k - upperlimit 
    # fcv_ul: shape = (sum i over n, k)
    fnik_nk = fnik[0] - n_k[0]
    for i in np.arange(num_of_obs-1):
        sec = fnik[i+1] - n_k[i+1]
        fnik_nk = np.concatenate((fnik_nk,sec))    
    fnik_nk = fnik_nk.astype('float64')
    # return shape(sum i over n,k)
    return fnik_nk

def calc_log_phi(constraining_vars,
                 upper_limit,
                 phi_scale,
                 cutoff_tol,
                 rows_to_obs):
    ...
    Parameters
    ----------
    constraining_vars : 2D ndarray.  shape = (sum i over n, k)
        All elements should by ints, floats, or longs.
    upper_limit : 2D ndarray. shape = (n,k)
        
    phi_scale : 1D ndarray. shape = (k)
        
    cutoff_tol : 1D ndarray. shape = (k)

    rows_to_obs : 2D ndarray. shape = (sum i over n, n)
    ...
    phi_tol = (1.0 / phi_scale) * np.log((1.0 - cutoff_tol)/cutoff_tol) #phi_tol: shape (k) 1D ndarray

    fcv_ul = calc_nik_minus_nk(constraining_vars,
                               upper_limit,
                               rows_to_obs)
    
    log_phi = -1 * np.log(1+np.exp(phi_scale * (fcv_ul + phi_tol)))

    # log_phi = -1 * (phi_scale * (fcv_ul + phi_tol))
    #return log_phi: (sum i over n, k) 2D ndarray
    return log_phi 


def calc_log_phi_summation(constraining_vars,
                           upper_limit,
                           phi_scale,
                           cutoff_tol,
                           rows_to_obs):
    
    log_phi = calc_log_phi(constraining_vars,
                           upper_limit,
                           phi_scale,
                           cutoff_tol,
                           rows_to_obs)

    log_phi_summation = np.sum(log_phi, axis=1) #summation over k for each individual/alternative: (sum i over n) 1D ndarray

    return log_phi_summation
...
def calc_probabilities(beta,
    ...
    Parameters
    ----------
    beta : 1D or 2D ndarray. shape = (index)
    ...
    design : 2D or 3D ndarray. shape = (sum i over n, index)
    ...
    # Calculate the systematic utility for each alternative for each individual  # 1D + 1D = 1D
    sys_utilities = design.dot(beta) + calc_log_phi_summation(constraining_vars,
                                                              upper_limit,
                                                              phi_scale,
                                                              cutoff_tol,
                                                              rows_to_obs)
...
def calc_gradient(beta,
    ...
    ##########################cmnl#####################################################
    # Calculate the gradient of the log-likelihood with respect to the upperlimits

    # shape (sum i over n, k)
    fcv_ul = calc_nik_minus_nk(constraining_vars,
                               upper_limit,
                               rows_to_obs)

    # phi_tol shape (k)
    phi_tol = (1.0 / phi_scale) * np.log((1.0 - cutoff_tol)/cutoff_tol)
    # phi shape (sum i over n, k)
    phi = (1/(1 + np.exp(phi_scale * (fcv_ul + phi_tol))))

    # ####reshape choice vector from (n*i) to (n,i,1)
    # cmnl_choice_vector = choice_vector.reshape(-1,num_option)[:,:,np.newaxis]
    # ####reshape choice vector from (n*i) to (n,i,1)
    # cmnl_long_probs = long_probs.reshape(-1,num_option)[:,:,np.newaxis]

    #dll_d_upperlimit is a 2D ndarray with shape (n,k)
    #dll_d_upperlimit = -1 * np.sum(cmnl_choice_vector * ((1 - phi) - np.sum((1 - phi) * cmnl_long_probs,axis = 1)[:,np.newaxis,:]), axis = 1) 
    #dll_d_phitol is a 1D ndarray with shape (k)

    #redundant
    #dll_d_phitol = np.sum(np.sum(cmnl_choice_vector * ((1 - phi) - np.sum((1 - phi) * cmnl_long_probs,axis = 1)[:,np.newaxis,:]), axis = 1),axis = 0)

    #big_phi for dll_d_phiscale (sum(i) over n,k)
    big_phi = ((1 - phi) * fcv_ul)

    cmnl_r_to_o = rows_to_obs.toarray()
    #dll_d_phiscale is a 1D ndarray with shape (k)
    # dll_d_phiscale = np.sum(cmnl_r_to_o.T.dot(choice_vector)[:,np.newaxis] * (cmnl_r_to_o.T.dot(big_phi) -
    #                  (cmnl_r_to_o.T.dot(big_phi * long_probs[:,np.newaxis]))),axis=0) 

    # shape (n,k)
    sum_big_phi_prob = cmnl_r_to_o.T.dot(big_phi * long_probs[:,np.newaxis])

    # calculate choice_vector*big_phi and delete zero vector
    choice_v_multiply_big_phi_b = choice_vector[:,np.newaxis] * big_phi
    non_chosen = np.where(choice_vector==0)
    choice_v_multiply_big_phi_a = np.delete(choice_v_multiply_big_phi_b, non_chosen, axis=0)

    # #shape (sum i over n, k)
    # fbig_phi_sum_big_phi_prob = calc_nik_minus_nk(big_phi,
    #                                               sum_big_phi_prob,
    #                                               rows_to_obs)

    # dll_d_phiscale = np.sum(choice_vector[:,np.newaxis] * fbig_phi_sum_big_phi_prob, axis=0)

    dll_d_phiscale = np.sum(choice_v_multiply_big_phi_a - sum_big_phi_prob, axis=0) 
    ...
    else: #shape_params and intercept_params neither is None   # dll_d_phitol is redundant
        gradient = np.concatenate((d_ll_d_beta, dll_d_phiscale), axis=None)
        params = beta
...
def calc_hessian(beta,
    ...
    dh_d_phi_scale = (-1 * (fcv_ul) * np.exp((phi_scale * (fcv_ul) * cutoff_tol - cutoff_tol + 1) /
                        cutoff_tol) / (1 + np.exp(phi_scale * (fcv_ul) + ((1-cutoff_tol) / cutoff_tol)))
                        )

    # dh_d_phi_scale = dh_d_phi_scale_n.reshape(-1, upper_limit.shape[1])

    # d2_ll_dps2 = -1 * dh_d_phi_scale.T.dot(dp_dh * dh_d_phi_scale)
    d2_ll_dps2 = -1 * quadratic_prod_wrt_dp_ds(
                      dh_d_phi_scale.T, dh_d_phi_scale, long_probs, rows_to_obs, weights=weights)

    # d2_ll_db_dps = -1 * dh_d_phi_scale.T.dot(dp_dh * dh_db)   
    d2_ll_db_dps = -1 * quadratic_prod_wrt_dp_ds(
                        dh_d_phi_scale.T, dh_db, long_probs, rows_to_obs, weights=weights)
    ...
    if shape_params is not None and intercept_params is not None:
        ...
        top_row = np.concatenate((d2_ll_db2,
                                  d2_ll_db_dps.T), axis=1)
        last_row = np.concatenate((d2_ll_db_dps,
                                   d2_ll_dps2), axis=1)
        hess = np.concatenate((top_row,
                               last_row), axis=0)
...
def calc_fisher_info_matrix(beta,
    ...
    # Differentiate the transformed utilities with respect to the phi_scales.
    # This should be a dense numpy array.
    fcv_ul = calc_nik_minus_nk(constraining_vars,
                               upper_limit,
                               rows_to_obs)
    ...
    if shape_params is not None and intercept_params is not None:
        ...
        gradient_vec = d_ll_dh.T * np.concatenate((dh_db,
                                                   dh_d_phi_scale), axis=1)
