...
def get_cmnl_point_est(orig_model_obj,
                      new_df,
                      boot_id_col,
                      num_params,
                      cmnl_spec,
                      ...
                      cmnl_fit_kwargs):
    """
    Calculates the MLE for the desired CMNL model.
    ...
    cmnl_spec : OrderedDict or None.
        If `orig_model_obj` is not a CMNL model, then `cmnl_spec` should be an
        OrderedDict that contains the specification dictionary used to estimate
        the CMNL model that will provide starting values for the final estimated
        model. If `orig_model_obj` is a CMNL model, then `cmnl_spec` may be None.
    cmnl_names : OrderedDict or None.
        If `orig_model_obj` is not a CMNL model, then `cmnl_names` should be an
        OrderedDict that contains the name dictionary used to initialize the
        CMNL model that will provide starting values for the final estimated
        model. If `orig_model_obj` is a CMNL, then `cmnl_names` may be None.
    cmnl_init_vals : 1D ndarray or None.
        If `orig_model_obj` is not a CMNL model, then `cmnl_init_vals` should be
        a 1D ndarray. `cmnl_init_vals` should denote the initial values used to
        estimate the CMNL model that provides starting values for the final
        desired model. If `orig_model_obj` is a CMNL model, then `cmnl_init_vals`
        may be None.
    cmnl_fit_kwargs : dict or None.
        If `orig_model_obj` is not a CMNL model, then `cmnl_fit_kwargs` should be
        a dict. `cmnl_fit_kwargs` should denote the keyword arguments used when
        calling the `fit_mle` function of the CMNL model that will provide
        starting values to the desired choice model. If `orig_model_obj` is a
        MNL model, then `cmnl_fit_kwargs` may be None.
    ...
    cmnl_point : dict.
        ...
        desired CMNL model.
    cmnl_obj : An CMNL model instance.
        The model object used to estimate the desired CMNL model.
    """
    ...
    # expected to be None.
    if orig_model_obj.model_type == model_type_to_display_name["CMNL"]:
        cmnl_spec = orig_model_obj.specification
        cmnl_names = orig_model_obj.name_spec
        if cmnl_init_vals is None:
            cmnl_init_vals = np.zeros(num_params)
        if cmnl_fit_kwargs is None:
            cmnl_fit_kwargs = {}

    # Alter the cmnl_fit_kwargs to ensure that we only perform point estimation
    cmnl_fit_kwargs["just_point"] = True
    # Use BFGS by default to estimate the CMNL since it works well for the CMNL.
    if "method" not in mnl_fit_kwargs:
        cmnl_fit_kwargs["method"] = "BFGS"

    # Initialize the cmnl model object for the given bootstrap sample.
    cmnl_obj = pl.create_choice_model(data=new_df,
                                     alt_id_col=orig_model_obj.alt_id_col,
                                     obs_id_col=boot_id_col,
                                     choice_col=orig_model_obj.choice_col,
                                     specification=cmnl_spec,
                                     model_type="CMNL",
                                     names=cmnl_names)

    # Get the CMNL point estimate for the parameters of this bootstrap sample.
    cmnl_point = cmnl_obj.fit_mle(cmnl_init_vals, **cmnl_fit_kwargs)
    return cmnl_point, cmnl_obj
